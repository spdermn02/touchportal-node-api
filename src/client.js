...
class TouchPortalClient extends EventEmitter {
	...
	createState(id, desc, defaultValue) {
		if (this.customStates[id]) {
			console.log(this.pluginId, `: ERROR : createState: Custom state of ${id} already created`);
	...
	choiceUpdate(id, value) {
		if (value.length <= 0) {
			console.log(this.pluginId, ': ERROR : choiceUpdate: value is an empty array');
	...
	choiceUpdateSpecific(id, value, instanceId) {
		if (value.length <= 0) {
			console.log(this.pluginId, ': ERROR : choiceUpdateSpecific : value does not contain data in an array format');
			throw new Error('choiceUpdateSpecific: value does not contain data in an array format');
		}
		if (!instanceId || instanceId == '') {
			console.log(this.pluginId, ': ERROR : choiceUpdateSpecific : instanceId is not populated');
	...
	stateUpdateMany(states) {
		...
		if (states.length <= 0) {
			console.log(this.pluginId, ': ERROR : stateUpdateMany : states contains no data');
	...
	sendArray(dataArray) {
		...
		if (dataArray.length <= 0) {
			console.log(this.pluginId, ': ERROR : sendArray : dataArray has no length');
		...
		this.socket.on('data', function(data) {
			...
			case 'closePlugin':
				if (message.pluginId === that.pluginId) {
					...
					console.log(that.pluginId, ': WARN : received Close Plugin message');
					that.socket.end();
					process.exit(9);
				}
				break;
			...
			case 'action':
				...
				break;
			default:
				console.log(that.pluginId, `: DEBUG : Unhandled type received ${message.type}`);
